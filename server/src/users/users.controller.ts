...
export class UsersController {
  ...
  async scheduleAccountDeletion(@CurrentUser() user: { userId: string }) {
    const deletionDate = await this.usersService.scheduleAccountDeletion(
      user.userId,
    );
    return {
      message:
  ...
  async getAccountStatus(@CurrentUser() user: { userId: string }) {
    const status = (await this.usersService.getAccountStatus(user.userId)) as {
      status: string;
      deletionDate?: Date;
    } | null;
    return status;
  ...
  async updatePrivacySettings(
    ...
    @Body() settings: Partial<Record<string, unknown>>,
  ) {
  ...
  async adminBanAccount(
    ...
    const deletionDate =
      await this.usersService.scheduleAccountDeletion(targetUserId);
    return {
