...
export class MediaService {
  ...
  constructor(
    ...
    this.logger.log(
      `Initializing S3 client with endpoint: ${endpoint}, region: ${region}, bucket: ${this.bucket}`,
    );
    this.logger.log(
  ...
  async createSignedDownloadUrl(
    ...
    if (this.publicBaseUrl) {
      return {
        url: `${this.publicBaseUrl.replace(/\/$/, '')}/${encodeURIComponent(key)}`,
        key,
      };
    }
  ...
  async uploadMediaBase64(
    ...
    try {
      this.logger.log(
        `Attempting upload to bucket: ${this.bucket}, key: ${key}`,
      );
  ...
  async setProfilePicture(
    ...
    if (!media.owner || media.owner.id !== userId)
      throw new ForbiddenException('Not your media');
    if (media.isNsfw)
      throw new BadRequestException('NSFW cannot be profile picture');
